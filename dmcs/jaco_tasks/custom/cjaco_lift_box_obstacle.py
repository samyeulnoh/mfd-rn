...
_LiftWorkspace = collections.namedtuple(
    "_LiftWorkspace", ["prop_bbox", "tcp_bbox", "target_bbox", "obs_bbox", "arm_offset"]
)
...
# reward parameters
_VERTICAL_MARGIN = 0.40 # 0.35 # 0.45 # 0.30 # 0.90 # 0.99
_FLOAT_MARGIN = 0.035  # box, obj_z(0.024975545806165192)
_OBJ_Z_MARGIN = 0.03  # box, obj_z(0.024975545806165192)

# pedestal parameters
_TARGET_RADIUS = 0.05
_PEDESTAL_RADIUS = 0.075 # 0.085 # 0.10 # 0.05 # 0.05
...
_BOX_WORKSPACE = _LiftWorkspace(
    ...
    target_bbox=workspaces.BoundingBox(
        # lower=(-0.1, -0.1, 0.18), upper=(0.1, 0.1, 0.18),
        lower=(-0.0, -0.0, 0.22), upper=(0.0, 0.0, 0.22),
    ),
...
class Lift(composer.Task):
    """A task where the goal is to elevate a prop with the constraint of an obstacle."""
    ...
    def get_reward(self, physics):
        ...
        obj_z_reward = (obj_z - _OBJ_Z_MARGIN) / (
            target[2] - _TARGET_Z_MARGIN - _OBJ_Z_MARGIN
        )
